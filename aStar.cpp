...
void aStar::pathFinderBishop(POINT startPos, POINT endPos, POINT currentPos, list<POINT>& lMyWay)
{
	//현재위치가 도착점이라면
	if (currentPos.x == endPos.x && currentPos.y == endPos.y)
	{
	...
	// 앙재귀띠
	pathFinderBishop(startPos, endPos, PointMake(tempTile->getIdx().x, tempTile->getIdx().y), lMyWay);
...
void aStar::pathFinder8(POINT startPos, POINT endPos, POINT currentPos, list<POINT>& lMyWay)
{
	//현재위치가 도착점이라면
	if (currentPos.x == endPos.x && currentPos.y == endPos.y)
	{
	...
	// 앙재귀띠
	pathFinder8(startPos, endPos, PointMake(tempTile->getIdx().x, tempTile->getIdx().y), lMyWay);
