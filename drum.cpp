...
void drum::imageInit()
{
	IMAGEMANAGER->addFrameImage("drum", L"images/monster/boss/coralriff/coralriff_drums.png", 328, 220, 4, 2);
	_img = IMAGEMANAGER->findImage("drum");

	EFFECTMANAGER->addEffect("enemy_attack", "images/effect/swipe_enemy.png", 270, 48, 54, 48, 5, 0.f, 10);

	KEYANIMANAGER->addAnimationType("drum");

	int stand[] = { 0,1,2,3 };
	KEYANIMANAGER->addArrayFrameAnimation("drum", "drum_Stand", "drum", stand, 4, ANIMATION_FPS ,true);

	int shadow[] = { 4,5,6,7 };
	KEYANIMANAGER->addArrayFrameAnimation("drum", "drum_Shadow", "drum", shadow, 4, ANIMATION_FPS, true);

	_ani = KEYANIMANAGER->findAnimation("drum", "drum_Stand");
	_ani->start();
