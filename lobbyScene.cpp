...
void lobbyScene::mapLoaderInit()
{
	_mapLoader = new mapLoader;

	_mapLoader->mapLoad(&_vvMap, &_vvObj, &_tileSizeX, &_tileSizeY, &_isBeat, 4);
}

void lobbyScene::lightMapInit()
{
	_vvLightMap.clear();
...
void lobbyScene::playerInit()
{
	_player = new player;
	_player->setMapAdressLink(&_vvMap);
	_player->setObjAdressLink(&_vvObj);
	_player->setPosCT({ (float)(_vvMap)[5][7]->getPos().x, (float)(_vvMap)[5][7]->getPos().y });
	_player->init();
}

void lobbyScene::rayCastInit()
{
...
void lobbyScene::beatInit()
{
	//_stageKeyName = "mapTool";
	_stageKeyName = "lobbyScene";
	_beatFileName = "sounds/zone/lobby.txt";
...
void lobbyScene::lightMapRender()
{
	for (int i = 0; i < _tileSizeY; i++)
	{
		for (int j = 0; j < _tileSizeX; j++)
