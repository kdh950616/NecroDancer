...
HRESULT stageBoss::init()
{
	...
	_mapLoader->makeBoss(&_vvObj, &_tileSizeX, &_tileSizeY,_pawnBeat);
...
void stageBoss::update()
{
	...
	_rayCast->rayCasting({ (int)_player->getPosCT().x / TILESIZE, (int)_player->getPosCT().y / TILESIZE }, 6);
...
void stageBoss::mapLoaderInit()
{
	...
	_mapLoader->mapLoad(&_vvMap, &_vvObj, &_tileSizeX, &_tileSizeY, &_isBeat, 3);
}

void stageBoss::lightMapInit()
{
	_vvLightMap.clear();
...
void stageBoss::beatInit()
{
	_stageKeyName = "bossScene";
	_beatFileName = "sounds/zone/boss_3.txt";
